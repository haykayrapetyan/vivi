...
- `pnpm dev:inngest` — optional: the Inngest dev server (queue for agent runs). Without it agent runs fall back to inline execution after the response — everything still works.
...
Inngest v4 (agent job queue) · react-markdown · next-themes (light/dark) ·
Vitest. UI is English-only.
...
`INNGEST_EVENT_KEY`/`INNGEST_SIGNING_KEY` — prod only (empty in dev). Local
...
- `src/lib/agent/` — the **per-vacancy autonomous recruiter agent**. `prompt.ts` builds ONE system prompt shared by the interactive chat and autonomous runs (creation rules + pool management + anti-injection trust rules + standing instructions); `tools.ts` — shared tools (`save_vacancy` (transactional), `list_candidates`, `get_candidate`), all scoped to the access-checked vacancy; `sanitize.ts` — `cleanUntrusted`/`untrustedBlock` fence candidate text in `<candidate_data>` tags; `run.ts` — `runCandidateCompleted()` (gated on `vacancy.status === "published"`: evaluate → analysis → post chat message with `source='auto'` → email; agent_run bookkeeping; agent_task idempotency, key recorded AFTER success so crashed runs retry); `store.ts` — DB ops incl. `ensureVacancyAgent` (lazy row, default suggest); `dispatch.ts` — send the Inngest event, returns false when unreachable; `keys.ts` — idempotency key builders. **There is no on/off toggle** — the agent is intrinsic to a vacancy and its activity follows the vacancy lifecycle (only a published vacancy is live). `vacancy_agent.enabled` stays as a DB-level emergency stop only.
- `src/lib/inngest/` — Inngest v4 client + functions (`createFunction` takes `triggers` in options, NOT a 3rd arg). `candidate-completed` has `concurrency: { key: vacancyId, limit: 1 }` — one agent run per vacancy at a time. Served at `/api/inngest`.
...
- `/interview/[token]` — candidate records webcam answers (`interview-client.tsx`, MediaRecorder; client retries upload on transient failure). `POST /api/interview/[token]/answer` saves + transcribes the clip; `.../complete` marks done and hands off to the agent: dispatch to Inngest, or (when unreachable) run inline via `after()` from `next/server` — either way the candidate gets an instant response. The agent run evaluates, posts the screening analysis to the vacancy chat and emails the recruiter (agent-review email when analysis was posted, the legacy completed-interview email when the agent is off or there's no `OPENAI_API_KEY`).
...
- **Inngest dev**: `pnpm dev:inngest` runs the local queue (auto-discovers `/api/inngest`). Without it, `inngest.send()` throws, `dispatch.ts` returns false and `/complete` runs the agent inline via `after()` — fine for dev, no retries/concurrency-keys though. In prod, deploy needs `INNGEST_EVENT_KEY`/`INNGEST_SIGNING_KEY` and a publicly reachable `/api/inngest`.
